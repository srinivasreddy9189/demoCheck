...
import { HomeSaveButton } from "./savevideobutton";
...
const token = Cookies.get('jwtToken')
        const url3 = `http://localhost:4005/homedetails/${homeId}`
        ...
        if(response.ok === true){
            ...
            setHomeDetails(data.homeDetails);
...
<Sidebar />
        ...
        <div className="col-md-9 col1">
            ...
            <ReactPlayer controls ={true} width='100%' height='500px' url={homeDetails.videoUrl} />
            <div className="mt-2 ml-4">
                <b>{homeDetails.description}</b>
            </div>
          <div className="d-flex justify-content-between ml-4">
            <div>
                <p>{homeDetails.views}</p>
            </div>
            <div>
                <p>{homeDetails.date}</p>
            </div>
    ...
    <button className={disliked ===true?'btn blue':'btn grey'} onClick={handleDisLikeClick}><AiOutlineDislike className=""/>
      ...
      <HomeSaveButton homeDetails= {homeDetails}/>
            </div>
            ...
            <div className='main d-flex mt-3 ml-4'>
                                <div className='sub1 mr-3'>
                                    <img src={homeDetails.channelLogo} className='img2'/>
                                </div>
                                <div className='sub2'>
                                   <p> <b>{homeDetails.channelName}</b></p>
                                   <p> <b>{homeDetails.subscribers}</b></p>
                                   <p> <b>{homeDetails.channelDescription}</b></p>
                                </div>
                            </div>
            </div>
