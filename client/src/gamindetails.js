...
const GamingDetails =()=>{
    ...
    const getGamingDetails = async()=>{
        setApiStatus(apiStatusConstant.progress);
const token = Cookies.get('jwtToken')
        const url3 = `http://localhost:4005/GamingDetails/${gamingId}`
        ...
        if(response.ok === true){
            ...
            setGamingDetails(data.gamingDetails);
    ...
    return(
        ...
        <div className="container-fluid">
            ...
            <ReactPlayer controls ={true} width='100%' height='500px' url={gamingDetails.videoUrl} />
            <div className="mt-2 ml-4">
                <b>{gamingDetails.description}</b>
            </div>
          <div className="d-flex justify-content-between ml-4">
            <div>
                <p>{gamingDetails.views}</p>
            </div>
            <div>
                <p>{gamingDetails.date}</p>
            </div>
    ...
    <button className={disliked ===true?'btn blue':'btn grey'} onClick={handleDisLikeClick}><AiOutlineDislike className=""/>
      ...
      <SaveVideoButton savedDetails={gamingDetails}/>
            </div>
            ...
            <div className='main d-flex mt-3 ml-4'>
                                <div className='sub1 mr-3'>
                                    <img src={gamingDetails.channelLogo} className='img2'/>
                                </div>
                                <div className='sub2'>
                                   <p> <b>{gamingDetails.channelName}</b></p>
                                   <p> <b>{gamingDetails.subscribers}</b></p>
                                   <p> <b>{gamingDetails.channelDescription}</b></p>
                                </div>
                            </div>
            </div>
                </div>
